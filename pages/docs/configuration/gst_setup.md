...
This feature allows users to automatically create a print format for documents
...
- Enable or disable the physical signature option. If the physical signature is
  disabled, the print format will display the Generated By information along
  with the creator's GSTIN. When enabled, a physical signature must be affixed
  manually on the document.

- Attach the file for your company logo to be displayed on the print format.

- **Bank Details**: Enter relevant information, including Account Number, Bank
  Name, UPI ID, Branch, IFSC Code, and any other necessary banking details.

- **Registration Details**: Input pertinent registration information such as
  MSME Number, MSME Type, LLPIN, LUT Number, or any other related details.

![Company Print Options](./assets/print_options_setup.png)

- Once all details are entered, the print format is created and ready for
  printing.
